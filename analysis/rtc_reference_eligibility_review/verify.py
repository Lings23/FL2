"""Read-only import audit of Linux H2 batch96 results; never prepares or starts training."""
...
from experiments import rtc_reference_eligibility_validation as validation
...
def audit():
    ...
    with zipfile.ZipFile(BATCH / 'r1e_sources.zip') as archive:
    ...
    relevant = ['defenses/rtc/spectral_direction.py', 'defenses/rtc/corroboration.py',
        ...
        'experiments/rtc_reference_eligibility_validation.py',
    ...
    # Compare audit code structurally; preparation-only CLI/client defaults differ by user request.
    def audit_ast(content, ignored):
        tree = ast.parse(content)
        tree.body = [n for n in tree.body if not (
            isinstance(n, ast.FunctionDef) and n.name in ignored or
            isinstance(n, ast.Assign) and any(isinstance(t, ast.Name) and t.id == 'CLIENT_CONTRACT' for t in n.targets))]
        return ast.dump(tree, include_attributes=False)
    for name in ('experiments/rtc_reference_eligibility_stage.py', 'experiments/rtc_i12_bridge.py'):
        assert audit_ast((ROOT/name).read_text(), {'command'}) == audit_ast(sources[name].decode(), {'command'}), name
    ...
    for cell in lock['cells']:
        ...
        assert cfg['ray']['client_num_gpus'] == .1, cell
    ...
    for name, attack, seed, ds in stage.batches():
        ...
        assert candidate['security']['defense']['custom_params']['reference_eligibility_mode'] == 'cap'
    ...
    def imported_csv(path):
        rows = validation.read_csv(path)
        for row in rows:
            for key in ('fit_rtc_r1e_quorum_numerator', 'fit_rtc_r1e_quorum_denominator'):
                if row.get(key):
                    value = integral_metadata(row[key])
                    if value != row[key]:
                        normalized_fields.append(dict(path=str(path), round=row['round'], field=key,
                            original=row[key], normalized=value))
                        row[key] = value
        return rows
    ...
    def imported_read(path):
        ...
        if p.is_file() and p.resolve().is_relative_to(BATCH):
            return read(p)
        marker = '/logs/rtc_reference_eligibility/'
        if marker in s:
    ...
    scope = dict(validation.verify_cell.__globals__, read_json=imported_read, read_csv=imported_csv,
    ...
    result.update(source_lock_sha256=sha((BATCH / 'r1e_lock.json').read_bytes()),
        source_archive_sha256=sha((BATCH / 'r1e_sources.zip').read_bytes()),
        ...
        client_contract=expected_client, ray_client_num_gpus=.1, path_mappings=sorted(mappings), training_environment=lock['environment'],
