"""H2 strict artifact validation: original R1c evidence and principal eligibility.
    ...
    for r in rr:
        ...
        for i, cid in enumerate(ids):
            ...
            if c['defense'] in RTC:
                ...
                mode = 'cap' if c['defense'] == 'rtc_i12_eligibility_cap' else 'observe'
                assert r['fit_rtc_r1e_mode'] == mode and r['fit_rtc_r1e_version'] == 'rtc_reference_eligibility.v1'
                assert int(r['fit_rtc_r1e_quorum_numerator']) == 2 and int(r['fit_rtc_r1e_quorum_denominator']) == 3
        ...
        if c['defense'] in RTC:
            eligibility_state.update({cid: truth(row['rtc_r1e_original_flagged']) or truth(row['rtc_r2_flagged']) for cid, row in clients.items()})
