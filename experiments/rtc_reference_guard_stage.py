"""H1 historical reference corroboration on accepted M2. All training is manual."""
import argparse
import copy
...
from experiments import rtc_reference_guard_validation as validator
from defenses.rtc import reference_guard
from experiments.rtc_reference_history_observation import verify_history
import statistics
import math

PARENT = 'rtc_i12_guard_observe'
OBSERVER = 'rtc_i12_guard_cap'
MK = 'rtc_i12_multikrum'
RFA = 'rtc_i12_rfa'
OUTPUT = ROOT / 'logs/rtc_reference_guard'
PROTOCOL = ROOT / 'config/rtc_reference_guard_protocol.json'
RECEIPT = ROOT / 'config/rtc_i12_accepted.json'
MODES = {PARENT: ('cap', 'cap'), OBSERVER: ('cap', 'cap'), MK: ('observe', 'observe'), RFA: ('observe', 'observe')}


def batches():
    return [(f'{attack}_seed{seed}', attack, seed, ds) for seed in (201,203)
...
def verify_lock(root):
    lock = prior.read_json(root / 'r1g_lock.json')
...
def prepare(root, data_dir=None):
    ...
    assert protocol['training_units'] == 24 and protocol['guard_version'] == reference_guard.VERSION
    ...
    for name, attack, seed, defenses in batches():
        ...
        assert observed['security']['defense']['custom_params']['reference_guard_mode'] == 'cap'
        observed['security']['defense']['custom_params']['reference_guard_mode'] = 'observe'
        assert observed == configs[PARENT], 'Reference guard mode must be the only configuration change'
    cal = prior.load_calibration(prior.CAL)
    raw = prior.raw_norm.load_calibration(prior.RAW_CAL)
    source_hashes = prior.sources()
    with zipfile.ZipFile(root / 'r1g_sources.zip', 'w', zipfile.ZIP_DEFLATED) as archive:
    ...
    lock = dict(schema='RTCReferenceGuardV1', stage='reference-guard', training_units=24,
    ...
    prior.write(root / 'r1g_lock.json', lock)
...
def metrics(run):
    ...
    if rtc:
        result.update(history_vetoed_benign=sum(prior.truth(c['rtc_r1g_vetoed']) for c in good),
            history_vetoed_malicious=sum(prior.truth(c['rtc_r1g_vetoed']) for c in bad),
...
def decide(runs,protocol):
    ...
    return dict(stage='reference-guard',quality_accepted=True,candidate_accepted=all(gates.values()),checks=gates,
        ...
        scope='IID seeds201 development and203 held-out engineering screen only',pending=protocol['pending'])
...
def analyze(root):
    ...
    for r in runs:
        if r['cell']['defense'] in (PARENT,OBSERVER):verify_history(r)
    result=decide(runs,lock['protocol'])
    result['source_lock_sha256']=prior.digest(root/'r1g_lock.json')
