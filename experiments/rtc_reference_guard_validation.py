"""H1 strict artifact validation: original R1c evidence and historical corroboration.
...
def verify_cell(root, c, lock):
    ...
    for r in rr:
        ...
        for i, cid in enumerate(ids):
            ...
            if c['defense'] in RTC:
                assert truth(row['rtc_r1g_original_flagged'])==flag
                assert float(row['rtc_r1g_original_q'])==(0. if flag and direction_mode=='cap' else 1.)
                usable=truth(row['rtc_r1h_valid'])
                assert truth(row['rtc_r1g_valid'])==usable
                if usable:
                    value=float(row['rtc_r1h_reference_cosine'])
                    assert math.isfinite(value) and abs(float(row['rtc_r1g_reference_cosine'])-value)<1e-10
                else:
                    assert row['rtc_r1g_reference_cosine']==''
                    value=None
                passes=usable and value>0.
                assert truth(row['rtc_r1g_passes'])==passes
                mode='cap' if c['defense']=='rtc_i12_guard_cap' else 'observe'
                assert r['fit_rtc_r1g_mode']==mode and r['fit_rtc_r1g_version']=='rtc_reference_history_guard.v1'
                assert float(r['fit_rtc_r1g_threshold'])==0.
                assert truth(row['rtc_r1g_vetoed'])==(mode=='cap' and flag and not passes)
                if mode=='cap':flag=flag and passes
