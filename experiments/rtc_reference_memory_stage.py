"""H2b confirmed reference memory on accepted M2. All training is manual."""
import argparse
import copy
...
BASE = 'rtc_i12_eligibility_observe'
PARENT = 'rtc_i12_eligibility_cap'
OBSERVER = 'rtc_i12_eligibility_confirmed'
MK = 'rtc_i12_multikrum'
RFA = 'rtc_i12_rfa'
OUTPUT = ROOT / 'logs/rtc_reference_memory'
PROTOCOL = ROOT / 'config/rtc_reference_memory_protocol.json'
...
def batches():
    return [(f'{attack}_seed{seed}', attack, seed, ds) for seed in (201,205)
...
def command(root, name, attack, seed, defenses, execute=False):
    ...
    # Shared host command edits must not change this registered H2 recipe.
    args = []
    index = 0
    ...
    args[args.index('--ray-client-num-gpus') + 1] = '.1'
    return args


def verify_lock(root):
    lock = prior.read_json(root / 'r1m_lock.json')
...
def prepare(root, data_dir=None):
    ...
    assert protocol['training_units'] == 32 and protocol['eligibility_version'] == reference_eligibility.VERSION
    ...
    for name, attack, seed, defenses in batches():
        ...
        assert observed['security']['defense']['custom_params'].pop('reference_eligibility_memory') == 'confirmed'
        assert observed == configs[PARENT], 'Only memory update semantics may differ from H2'
        observed['security']['defense']['custom_params']['reference_eligibility_mode'] = 'observe'
        assert observed == configs[BASE], 'H2 versus M2 may only change eligibility mode'
    cal = prior.load_calibration(prior.CAL)
    raw = prior.raw_norm.load_calibration(prior.RAW_CAL)
    source_hashes = prior.sources()
    with zipfile.ZipFile(root / 'r1m_sources.zip', 'w', zipfile.ZIP_DEFLATED) as archive:
    ...
    lock = dict(schema='RTCReferenceMemoryV1', stage='reference-memory', training_units=32,
    ...
    prior.write(root / 'r1m_lock.json', lock)
...
def decide(runs, protocol):
    ...
    assert set(index) == {(a,s,d) for _,a,s,ds in batches() for d in ds} and len(runs) == 32
    summaries = [metrics(r) for r in runs]
    table = {(r['attack'],r['seed'],r['defense']):r for r in summaries}
    gates, effects, divergences = {}, [], []
    g = protocol['gates']
    for name,attack,seed,ds in batches():
        candidate = table[attack,seed,OBSERVER]
        gates[name+'/candidate_benign_union'] = candidate['benign_union_flag_rate'] <= g['candidate_benign_union_max']
        for defense in ds:
            prior.paired(index[attack,seed,BASE],index[attack,seed,defense])
        for ref,label in ((BASE,'M2'),(PARENT,'H2')):
            parent = table[attack,seed,ref]
            a,b = index[attack,seed,ref],index[attack,seed,OBSERVER]
            ...
            for key in ('active_accuracy','final_accuracy'):
                ...
                gates[name+'/'+label+'/'+key] = delta >= g['accuracy_noninferiority_min']
                effects.append(dict(attack=attack,seed=seed,reference=label,metric=key,candidate_minus_reference=delta))
            if attack != 'none':
                gates[name+'/'+label+'/malicious_weight'] = candidate['malicious_weight_per_round'] <= parent['malicious_weight_per_round'] + g['malicious_weight_increase_max']
        ...
        gates[name+'/preserve_H2_benign_flags'] = candidate['benign_union_flags'] <= table[attack,seed,PARENT]['benign_union_flags']
        if attack == 'sign_flip' and seed == 201:
            gates[name+'/strict_benign_reduction_vs_M2'] = candidate['benign_union_flags'] < table[attack,seed,BASE]['benign_union_flags']
            gates[name+'/strict_malicious_veto_reduction_vs_H2'] = candidate['eligibility_vetoed_malicious'] < table[attack,seed,PARENT]['eligibility_vetoed_malicious']
    return dict(stage='reference-memory',quality_accepted=True,candidate_accepted=all(gates.values()),checks=gates,
        summaries=summaries,effects=effects,divergences=divergences,accepted_parent=BASE,research_parent=PARENT,candidate=OBSERVER,
        retained_candidates=['M2-G1','M2-H2'],final_goal_achieved=False,
        scope='IID batch96 seeds201 development and205 new engineering screen; H2 remains unaccepted until repair passes',
        pending=protocol['pending'])


def analyze(root):
    lock=verify_lock(root)
    scope=dict(validator.verify_cell.__globals__,MODES=MODES,RTC=(BASE,PARENT,OBSERVER))
    ...
    result['source_lock_sha256']=prior.digest(root/'r1m_lock.json')
