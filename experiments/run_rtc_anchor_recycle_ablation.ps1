param(
    ...
    [double]$RecycleFraction = 1.0
)

$ErrorActionPreference = 'Stop'
$workspace = Split-Path -Parent $PSScriptRoot
$python = Join-Path $workspace '.venv\Scripts\python.exe'
$freeze = Join-Path $workspace 'config\rtc_v3_anchor_recycle_ablation.freeze.json'
$output = Join-Path $workspace 'logs\rtc_v3_anchor_recycle_ablation_seed42'
...
$arguments = @(
    ...
    '--attacks', 'lie,dba',
    '--defenses', 'rtc_full,rtc_anchor_recycle',
    '--rtc-v3-anchor-recycle-fraction', $RecycleFraction.ToString(
...
if ($LASTEXITCODE -ne 0) {
    throw "RTC anchor-recycle ablation failed with exit code $LASTEXITCODE"
}
...
if ($Execute) {
    ...
    Write-Output 'Run again with -Execute to start the four training cells.'
}
