...
$output = Join-Path $workspace 'logs\rtc_v3_anchor_recycle_b3r_f051_seed42_mf03'
$manifestPath = Join-Path $output 'experiment_manifest.json'
$expectedManifestSha256 = '4ca5a8e4415de4dbfb8389d53748e53911b98685d9b7b0ed8dd0154518f42af2'
...
function Assert-B3RF051Manifest {
    if (-not (Test-Path -LiteralPath $manifestPath)) {
        throw "B3R-F0.51 manifest not found: $manifestPath. Run this script once without -Execute."
    }
    ...
    if ($expectedManifestSha256) {
        ...
        if ($actualManifestSha256 -ne $expectedManifestSha256) {
            throw "B3R-F0.51 manifest hash changed: expected=$expectedManifestSha256 actual=$actualManifestSha256"
        }
    }
    ...
    foreach ($attack in $expectedTrialPlanHashes.Keys) {
        ...
        if ($matches.Count -ne 1) {
            throw "Expected exactly one B3R-F0.51 spec for attack=$attack, found=$($matches.Count)"
        }
        $spec = $matches[0]
...
if ($Execute) {
    Assert-B3RF051Manifest
}

Set-Location -LiteralPath $workspace
& $python @arguments
if ($LASTEXITCODE -ne 0) {
    throw "RTC B3R-F0.51 ablation failed with exit code $LASTEXITCODE"
}
...
if ($Execute) {
    ...
    Write-Output 'Run again with -Execute to start the two candidate-only B3R-F0.51 cells.'
}
