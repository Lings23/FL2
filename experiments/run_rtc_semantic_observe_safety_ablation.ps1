...
$freeze = Join-Path $workspace 'config\rtc_v3_semantic_observe_safety.freeze.json'
$output = Join-Path $workspace 'logs\rtc_v3_semantic_observe_safety_seed42_mf03'
...
if ($LASTEXITCODE -ne 0) {
    throw "RTC semantic-observe safety ablation failed with exit code $LASTEXITCODE"
}
...
if ($Execute) {
    ...
    Write-Output 'Run again with -Execute to start the four paired training cells.'
}
