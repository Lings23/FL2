...
$freeze = Join-Path $workspace 'config\rtc_v3_semantic_risk_gated.freeze.json'
$output = Join-Path $workspace 'logs\rtc_v3_semantic_restricted_only_seed42_mf03'
...
$arguments = @(
    ...
    '--attacks', 'lie,dba',
    '--defenses', 'rtc_semantic_restricted_only',
...
if ($LASTEXITCODE -ne 0) {
    throw "RTC semantic restricted-only ablation failed with exit code $LASTEXITCODE"
}
...
if ($Execute) {
    ...
    Write-Output 'Run again with -Execute to start the two candidate-only training cells.'
}
