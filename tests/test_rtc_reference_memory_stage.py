...
@pytest.mark.parametrize('batch_args', [['--batch-size', '96'], ['--batch-size=96'],
    ['--batch-size', '96', '--batch-size=128'], []])
def test_h2_command_pins_batch_after_shared_host_overrides(tmp_path, monkeypatch, execute, batch_args):
    from experiments.run import parse_args
    inherited = stage.prior.command(tmp_path, 'none_seed201', 'none', 201, (stage.PARENT,), execute=execute)
    pos = inherited.index('--batch-size')
    inherited[pos:pos + 2] = batch_args
    inherited[inherited.index('--ray-client-num-gpus') + 1] = '.5'
    saved = list(inherited)
    monkeypatch.setattr(stage.prior, 'command', lambda *a, **kw: inherited)
    result = stage.command(tmp_path, 'none_seed201', 'none', 201, (stage.PARENT,), execute=execute)
    parsed = parse_args(result[3:])
    assert parsed.batch_size == 96 and parsed.ray_client_num_gpus == .1
    assert result.count('--batch-size') == 1 and not any(x.startswith('--batch-size=') for x in result)
    assert parsed.dry_run == (not execute)
    assert execute or result[-1] == '--dry-run'
    assert inherited == saved
...
def test_matrix_defaults_and_failed_run_guard(tmp_path,monkeypatch):
    ...
    assert {s for _,_,s,_ in stage.batches()}=={201,205}
...
def test_repair_requires_safety_reduction_and_preserves_utility_and_weight_gates(monkeypatch):
    ...
    for name,attack,seed,ds in stage.batches():
        for d in ds:
            ...
            m=dict(attack=attack,seed=seed,defense=d,active_accuracy=.83,final_accuracy=.88,
                ...
                malicious_weight_per_round=.04,eligibility_vetoed_malicious=0 if candidate else 3)
    ...
    for key,value in [('active_accuracy',.82),('final_accuracy',.87),('benign_union_flag_rate',.02),('malicious_weight_per_round',.05),('benign_union_flags',4),('eligibility_vetoed_malicious',3)]:
...
def test_full_validator_replays_principal_state_and_rejects_forgery(combined_artifacts):
    ...
    for r in rr[1:]:
        ...
        h.commit(h.transition_confirmed(ids,original,[dict(rtc_r2_flagged=v.truth(a['rtc_r2_flagged'])) for a in clients],extra))
        r.update(fit_rtc_r1e_version=VERSION,fit_rtc_r1e_mode='cap',fit_rtc_r1e_quorum_numerator=2,fit_rtc_r1e_quorum_denominator='3.0',fit_rtc_r1e_memory='confirmed')
    ...
    scope=dict(v.verify_cell.__globals__,MODES=stage.MODES,RTC=(stage.BASE,stage.PARENT,stage.OBSERVER))
